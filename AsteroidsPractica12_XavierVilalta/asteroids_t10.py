...
class World(object):
    ...
    def handle_keyup(self, event):
        ...
        if event.key == pygame.K_SPACE:
            bullet = Bullet((Vector(*self.player.rect.center)-self.player.facing*2).to_position(),self.player.facing.to_degrees()[0]+180,self.player.motion.magnitude() + 8)
...
def colisio_nau():
    for i in ([x for x in world.sprites if isinstance(x, Asteroid)]):
        if abs(i.rect.center[0]-world.player.rect.center[0])<15:
            if abs(i.rect.center[1]-world.player.rect.center[1])<18:
...
def main():
    ...
    supdate.start()
    updateCollisions = Thread(target = update_collisions)
    updateCollisions.start()
    while world.running:
